...
export default function DashboardOrdersTable({ orders }: DashboardOrdersTableProps) {
  ...
  return (
    <div className="min-h-0 flex-1 overflow-x-auto border border-slate-200 bg-white shadow-sm">
      ...
      >
        <colgroup>
          <col className={columnWidths ? undefined : "w-[12.5%]"} style={columnWidths ? { width: columnWidths[0] } : undefined} />
          <col className={columnWidths ? undefined : "w-[12.5%]"} style={columnWidths ? { width: columnWidths[1] } : undefined} />
          <col className={columnWidths ? undefined : "w-[12.5%]"} style={columnWidths ? { width: columnWidths[2] } : undefined} />
          <col className={columnWidths ? undefined : "w-[12.5%]"} style={columnWidths ? { width: columnWidths[3] } : undefined} />
          {PROGRESS_STEPS.map((step, index) => (
            <col className={columnWidths ? undefined : "w-[7.143%]"} key={step} style={columnWidths ? { width: columnWidths[index + 4] } : undefined} />
          ))}
        </colgroup>
        <thead className="bg-slate-50">
          <tr className="border-b border-slate-200 text-slate-900">
            {["발주번호", "고객사", "품명", "발주수량"].map((label, index) => (
